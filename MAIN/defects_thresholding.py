...
for file in glob.glob(path):
    ...
    label_final = morphology.remove_small_objects(label_image, min_size=2000)
    label_final_var = morphology.remove_small_objects(label_image_var, min_size=2000)
    if label_final.any()!=0  and label_final_var.any() !=0:
        ...
        props_df['error'] = abs(round((props_df['equivalent_diameter'] - props_df_var['equivalent_diameter'])*pixels_to_microns))
        props_df['img'] = i
        data.append(props_df)
        print('defects_df')
        print(props_df)
        print('error')
        print(props_df['error'])
    ...
    if cleaned_holes.any()!= 0 and cleaned_holes_var.any() != 0:
        ...
        holes_df['error'] = abs(round((holes_df['equivalent_diameter'] - holes_df_var['equivalent_diameter'])*pixels_to_microns))
        holes_df['img'] = i
        holes_data.append(holes_df)
        print('holes_df')
        print(holes_df)
        print('error holes')
        print(holes_df['error'])
    print(file, i)
    i += 1
